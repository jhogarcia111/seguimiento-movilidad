...
async function createTables(pool) {
  try {
    ...
    await promoteBootstrapAdminUsers(pool);
...
/**
 * Promueve usuarios existentes a admin + cuenta activa.
 * - Si `BOOTSTRAP_ADMIN_USERNAMES` no está definida: por defecto `Jho` (proyecto Transito Tito).
 * - Si está definida vacía: no hace nada (útil en forks; pon `BOOTSTRAP_ADMIN_USERNAMES=`).
 * - Si es lista separada por comas: aplica a cada username (case-insensitive).
 */
function getBootstrapAdminUsernames() {
  if (Object.prototype.hasOwnProperty.call(process.env, 'BOOTSTRAP_ADMIN_USERNAMES')) {
    const v = process.env.BOOTSTRAP_ADMIN_USERNAMES;
...
async function promoteBootstrapAdminUsers(pool) {
  const names = getBootstrapAdminUsernames();
  ...
  for (const name of names) {
    try {
      ...
      if (r.rowCount > 0) {
        console.log(`✅ Usuario "${name}" promovido a administrador (bootstrap)`);
      }
    } catch (err) {
      console.error(`Error promoviendo usuario bootstrap "${name}":`, err.message);
    }
  }
}
