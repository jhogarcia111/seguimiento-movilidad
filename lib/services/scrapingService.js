    ...
    try {
      ...
      for (const blogpost of validatedBlogposts) {
        ...
        try {
          ...
          // Pasar blogpost.date para que los timestamps reflejen la fecha real del artículo
          const blogpostNews = await scrapeBlogpostContent(blogpost.url, blogpost.title, userQuery, needsReview, blogpost.date);
 ...
 */
async function scrapeBlogpostContent(url, title, userQuery = null, needsContentReview = false, blogpostDate = null) {
  try {
    ...
    if (blogpostDate) {
      console.log(`   📅 Fecha de publicación del blogpost: ${blogpostDate.toISOString()}`);
    }
    ...
    // Pasar needsContentReview para indicar que se debe revisar contenido aunque el título no sea relevante
    // Pasar blogpostDate para que los timestamps reflejen la fecha real del artículo, no la del scraping
    const news = await extractNewsFromHTML(response.data, url, userQuery, needsContentReview, blogpostDate);
 ...
 */
export async function extractNewsFromHTML(html, baseUrl, userQuery = null, needsContentReview = false, blogpostDate = null) {
  const $ = cheerio.load(html);
  const news = [];

  // Timestamp por defecto: la fecha de publicación del artículo si está disponible,
  // sino la fecha de scraping. Esto evita que noticias antiguas aparezcan como "hace pocas horas".
  const ARTICLE_TIMESTAMP = (() => {
    if (blogpostDate instanceof Date && !Number.isNaN(blogpostDate.getTime())) {
      return blogpostDate.toISOString();
    }
    // Intentar extraer fecha de publicación del propio HTML (meta tags, <time>, etc.)
    const metaDate =
