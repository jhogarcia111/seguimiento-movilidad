...
export default function HomePage() {
  ...
  const { getRandomMessage, getRandomButtonText } = useTitoModal();
  const { registerPageVideo, unregisterPageVideo } = useVideoContext();

  const [usedVideos] = useState(() => {
    ...
    const modalVideo =
      remainingWelcomeVideos[Math.floor(Math.random() * remainingWelcomeVideos.length)];
    const clearVideo = allClearVideos[Math.floor(Math.random() * allClearVideos.length)];

    return { hero: heroVideo, modal: modalVideo, clear: clearVideo };
  });
      ...
      <div className="hero">
        <div className="hero-content">
          <div className="hero-text">
            ...
            <Link
              href={isAuthenticated ? '/buscar' : '/login'}
        ...
        {isLoading && (
          <>
            ...
            <p className="section-subtitle">
              Estos son algunos de los problemas de movilidad más recientes reportados en la
              ciudad. Ejemplo de cómo se verían los resultados de una búsqueda.
            </p>
          ...
          <>
            {data.incidents && data.incidents.length > 0 ? (
              <>
                ...
                <div
                  className={`incident-count-header ${data.isMock ? 'mock-data' : ''}`}
                >
                  {data.count} problema{data.count !== 1 ? 's' : ''} reportado
                  {data.count !== 1 ? 's' : ''} recientemente
                  {data.source === 'cache' && (
              ...
              <div className="no-results">
                <div className="no-results-content">
                  <div className="no-results-text">
                    <h2>📊 Últimos Problemas de Movilidad Reportados en Bogotá</h2>
                    <p className="section-subtitle">
                      Estos son algunos de los problemas de movilidad más recientes reportados en
                      la ciudad. Ejemplo de cómo se verían los resultados de una búsqueda.
                    </p>
                    <div className="no-results-message-container">
                      ...
                      <p className="no-results-note">
                        Todo está normal en la ciudad. Si hay tráfico, puede ser por flujo normal
                        de vehículos.
                      </p>
                    </div>
                  </div>
