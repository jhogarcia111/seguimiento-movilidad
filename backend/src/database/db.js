...
/**
 * Convierte placeholders ? (estilo mysql2) a $1, $2, ... para PostgreSQL
 */
...
/**
 * Ejecuta SQL con la misma forma que mysql2: const [rows] = await pool.execute(...)
 * - SELECT → [rows]
 * - INSERT → [{ insertId, affectedRows }] (requiere RETURNING id; se añade automáticamente)
 * - UPDATE/DELETE → [{ affectedRows, insertId: 0 }]
 */
...
/**
 * Inicializa el pool de PostgreSQL (Neon u otro proveedor compatible)
 */
export async function initDatabase() {
  ...
  if (!connectionString || !connectionString.startsWith('postgres')) {
    throw new Error(
      'DATABASE_URL debe ser una cadena postgresql://... (configúrala en backend/.env). MySQL ya no está soportado en este proyecto.'
    );
  }
