...
function SectorSearch() {
  ...
  // Inicializar el estado basado en sessionStorage para evitar mostrar el modal antes de verificar
  const hasShownModalInSession = typeof window !== 'undefined' && sessionStorage.getItem('hasShownSearchTitoModal');
  const [showInitialModal, setShowInitialModal] = useState(!hasShownModalInSession);
  const [showSplitLayout, setShowSplitLayout] = useState(!!hasShownModalInSession);
  ...
  useEffect(() => {
    ...
    // Crear URL base
    const baseURL = api.defaults.baseURL || 'http://localhost:3051';
    const url = `${baseURL}/api/user/search`;
